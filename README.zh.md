...
- **一个 baseURL，四个供应商路由**：`sub2api-openai`、`sub2api-claude`、`sub2api-grok`、`sub2api-gemini`——各自配置独立 key 与至少一个模型，两者就绪后即注册为可用的 LLM 供应商。
...
- **推理等级（思考模式）**：对话模型选择器可直接调整 `reasoning_effort`（透传网关）；设置页「思考强度」字段按 [models.dev](https://models.dev/) 的 `reasoning_options` 逐模型填充真实档位（如 `gpt-5.6-sol` 为 none/low/medium/high/xhigh/max，`deepseek-v4-flash` 为 low/high/max），设置页只读展示；可在 settings.yaml 中用 `reasoningEfforts: []` 显式关闭。
...
- **Auto Vision（自动识图）**：给纯文本模型图片能力。**不仅覆盖本插件自己的 `sub2api-*` 路由，还自动包装所有已注册的文本模型提供商**——deepseek 官方（`deepseek-official`）、其它 `llm-pi-ai:` 配置添加的路由、其他插件添加的路由都会生成同名镜像（如 `deepseek-official-vision`，模型选择器中显示为「DeepSeek + 自动识图」）。镜像路由声明图片输入，通过 harness 附件准入后，把图片块改写为视觉模型转述（走设置的识图模型，按附件缓存），再以纯文本回合委托给原模型——原文本模型始终只收文本，视觉模型只当眼睛。跟随 `llm/adapters-updated` 事件自动增删镜像，与其他插件的同名路由冲突时自动跳过。
